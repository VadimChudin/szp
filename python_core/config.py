...
# Ниже значение перечитывается из .env (ключ CLUSTER_TOLERANCE). От шага
# «лестницы» оно больше не зависит — см. блок «Полоса отображения зон».
...
# Локальный footprint-буфер зависит от времени запуска приложения. Чтобы зоны
# совпадали на разных ПК, он по умолчанию не создаёт базовые уровни.
FOOTPRINT_LEVELS_IN_ZONES = _env_bool("FOOTPRINT_LEVELS_IN_ZONES", False)

# ── Ограничение вывода ──────────────────────────────────────────────────────
# На графике действует один общий MAX_ZONES_ON_CHART без квоты по сторонам.
# ZONES_PER_SIDE / MIN_ZONES_PER_SIDE удалены: схема «3 сверху + 3 снизу» убрана
# из обоих режимов отбора, и константы никем не читались — они лишь путали при
# поиске причины, почему зоны не показываются.
#
# Режим жизненного цикла отображаемых зон:
#   False (по умолчанию) — process_legacy_zones: топ по score, сжигание пробитых
#                          зон, историчные тусклее (HIST). Это то, что у клиента.
#   True                 — H4-снапшот (active_zones.update_snapshot): состав
#                          меняется только после закрытия H4-свечи.
# Квоты по сторонам нет ни в одном из режимов: оба режут только по
# MAX_ZONES_ON_CHART, поэтому вернуть 3+3 через .env невозможно.
...
MAX_ZONES_ON_CHART = max(1, min(500, _env_int("MAX_ZONES_ON_CHART", 6)))
...
# ZONE_BAND_OUTER_MAX удалён: константа считалась от ZONES_PER_SIDE и нигде не
# использовалась. Видимую область задаёт скоп (ZONE_SCOPE_PIPS), см. ниже.

# Склейка близких уровней и ширина зоны — явные значения в долларах.
# Раньше они выводились из ZONE_GAP_MIN (шага «лестницы» 3+3): скоп и лимит
# зон от лестницы уже не зависят, а склейка продолжала молча тянуться за ней,
# то есть менялась при правке параметров выключенного режима.
# Числа подобраны так, чтобы эффективные значения не изменились:
#   CLUSTER_TOLERANCE = $5.0, ZONE_WIDTH_MAX = $7.0.
CLUSTER_TOLERANCE = _env_float("CLUSTER_TOLERANCE", 5.0)   # ~50 пунктов
ZONE_WIDTH_MAX = min(ZONE_WIDTH_MAX, 7.00)
...
# Скоп показа в пунктах — ВСЯ ширина окна. Любое положительное число: половина вверх, половина вниз.
# ATR-окно выключено по умолчанию: клиент задаёт скоп руками, не «как получится».
ZONE_WINDOW_ATR = _env_float("ZONE_WINDOW_ATR", 0.0)
ZONE_SCOPE_PIPS = max(0.0, _env_float("ZONE_SCOPE_PIPS", 800.0))
...
# ── Слой ИИ (локальная модель Qwen) ─────────────────────────────────────────
# Надстройка над готовым индикатором, а не второй алгоритм. Модель читает уже
# посчитанные зоны, ранжирует их и подписывает человеческой фразой. Цена зоны,
# ширина, окно показа, вердикт confirm_zones и уровень SL остаются за кодом:
# канон «одинаковые зоны у всех брокеров» держится на детекторе и эталоне
# Dukascopy, а языковая модель недетерминирована.
#
# Требуется ключ продукта (см. ai/licensing.py). Истёк ключ, нет модели, нет
# ответа — индикатор работает в обычном режиме без каких-либо изменений.
AI_ENABLED = _env_bool("AI_ENABLED", True)

# Сколько зон-кандидатов уходит в модель за один вызов. Включая отброшенные
# кодом: иначе модель просто соглашается с фильтром вместо того, чтобы спорить.
# На длинном списке качество ранжирования падает — 20 это разумный потолок.
AI_MAX_CANDIDATES = _env_int("AI_MAX_CANDIDATES", 20)

# Сколько раз за вызов разрешено считать историческую статистику по аналогам.
# Расчёт идёт по всей истории Dukascopy и стоит заметного времени.
AI_ANALOG_CALLS = _env_int("AI_ANALOG_CALLS", 3)

AI_MAX_TOKENS = _env_int("AI_MAX_TOKENS", 700)

# Модель вызывается раз в закрытую свечу H4 — 6 раз в сутки. Латентность не
# важна совсем, поэтому выбирается самая крупная модель, какую тянет железо,
# а не самая быстрая. Пустое значение = определять автоматически по конфигу ПК.
AI_MODEL_OVERRIDE = _env_str("AI_MODEL_OVERRIDE", "")
