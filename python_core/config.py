...
# Детерминированный пересчёт (умолчание вкл): на закрытии каждой H4-свечи
# снапшот пересобирается ТОЛЬКО из свежей детекции — без переноса прошлых
# зон из снапшота, без POC-бонусов из локальной тиковой истории. Без этого
# два клиента у ОДНОГО брокера видели разные зоны: снапшот инкрементальный
# (старые зоны держат слоты), а тики копятся локально с момента установки —
# состав зависел от того, когда клиент запустил сборку. В режиме on закрытие
# H4 даёт всем клиентам одинаковый вход → одинаковый выход. Стабильность
# внутри бара сохраняется: кэш на том же H4 отдаётся без пересчёта, как раньше.
DETERMINISTIC_RECALC = _env_bool("DETERMINISTIC_RECALC", True)
# POC/HVN-бонусы из локального футпринта (шаг 1.5 детектора). Полезны на
# одиночной установке, но делают score зависимым от аптайма клиента, поэтому
# при DETERMINISTIC_RECALC отключаются принудительно.
INCLUDE_FOOTPRINT_POC = _env_bool("INCLUDE_FOOTPRINT_POC", True)

# Фильтр пробитых уровней. Детектор отвечает на вопрос «где цена отбивалась»,
# но без этого фильтра не спрашивает, что стало с уровнем ПОТОМ. Уровень,
# через который тело H4 прошло насквозь после последнего касания, свой запас
# лимитных заявок потратил — он съеден. Клиентский контракт «зона исчезает
# при пробое телом» раньше работал только для уже видимого снапшота
# (active_zones._invalidate), а свежие кандидаты от детектора пробитие
# игнорировали: после обвала график засыпало «затычками» — линиями, через
# которые цена уже прошла. Фильтр снимает такие кандидаты до отбора.
BROKEN_ZONE_FILTER = _env_bool("BROKEN_ZONE_FILTER", True)
...
# ── SL-уровни в экспорте ────────────────────────────────────────────────────
# Клиент просил «только зоны». SL — это информационный кандидат ликвидности,
# не зона и не сигнал. По умолчанию НЕ экспортируем: если поля sl нет в JSON,
# ни терминал, ни footprint физически не смогут его нарисовать.
EXPORT_SL_LEVELS = _env_bool("EXPORT_SL_LEVELS", False)
...
# Слой подтверждения (zone_confirmation) отвечает на вопрос «жив ли уровень»
# по ИСТОРИИ: профиль объёма, пулы стопов, дельта прошлых касаний. L2-валидация
# отвечает на тот же вопрос по НАСТОЯЩЕМУ: что прямо сейчас стоит в стакане.
# Это единственный источник, где видны ещё не исполненные намерения — лимитные
# заявки до того, как они стали сделками.
#
# Источник данных: MT5 Collector EA снимает стакан через MarketBookGet и пишет
# l2_book.json. У MT4 стакана нет — там слой честно уходит в UNAVAILABLE и
# не влияет на выдачу. У части брокеров XAU/USD стакан пуст или покрывает
# лишь пару долларов от цены — это тоже UNAVAILABLE, а не выдуманный ноль.
#
#   off      — слой выключен
#   annotate — считаем и пишем в JSON/подпись, состав зон НЕ меняем (умолчание)
#   filter   — зоны с вердиктом DEAD убираются (UNAVAILABLE не трогаем никогда)
#   rerank   — зоны сортируются по score * (0.5 + l2)
L2_MODE = _env_str("L2_MODE", "annotate")
# Метка L2 в подписи зоны на графике: «L2✓0.82» / «L2~0.55» / «L2✗0.21».
L2_IN_LABEL = _env_bool("L2_IN_LABEL", True)

# Веса проверок. Стена в стакане — прямое наблюдение, её вес максимальный;
# перекос книги — рыночный контекст; стойкость стены (спуфинг-фильтр) —
# производная оценка, ей минимум.
L2_WEIGHT_WALL        = _env_float("L2_WEIGHT_WALL", 0.45)
L2_WEIGHT_IMBALANCE   = _env_float("L2_WEIGHT_IMBALANCE", 0.30)
L2_WEIGHT_PERSISTENCE = _env_float("L2_WEIGHT_PERSISTENCE", 0.25)

# Пороги вердикта. UNAVAILABLE выставляется до скоринга и пороги не проходит.
L2_LIVE_THRESHOLD = _env_float("L2_LIVE_THRESHOLD", 0.65)
L2_DEAD_THRESHOLD = _env_float("L2_DEAD_THRESHOLD", 0.40)

# Стеной считаем уровень, чей объём минимум во столько раз превышает медианный
# объём видимых уровней своей стороны книги.
L2_WALL_STRONG = _env_float("L2_WALL_STRONG", 2.5)
L2_WALL_WEAK   = _env_float("L2_WALL_WEAK", 1.0)

# Полуширина окна вокруг mid-цены (в долларах), в котором считается перекос
# bid/ask. Дальше этого окна заявки на перекос почти не влияют.
L2_IMBALANCE_REACH_USD = _env_float("L2_IMBALANCE_REACH_USD", 15.0)

# Стакан старше этого возраста (сек) считается мёртвым: брокер мог закрыть
# торги или EA отвалился — доверять такому снапшоту нельзя.
L2_MAX_AGE_SEC = _env_int("L2_MAX_AGE_SEC", 300)

# Явный путь к l2_book.json (для тестов и нестандартных установок).
# Пусто — ищем сами: data_bridge, затем MQL5/Files всех найденных терминалов.
L2_BOOK_PATH = _env_str("L2_BOOK_PATH", "")

# ── Внешний источник стакана (прокси-L2) ───────────────────────────────────
# У спотового XAU/USD нет единого стакана: DOM брокера — его внутренняя
# кухня, и у части брокеров он пустой. Тогда валидацию может кормить внешняя
# книга по токенизированному золоту PAXG (публичные REST, без ключей):
# цены калибруются сдвигом к брокерской mid, объёмы нормируются самой книгой.
# Внешняя книга — ПРОКСИ: используется только когда своего стакана нет,
# и всегда помечена в отчёте как proxy.
#   off     — не ходить за внешним стаканом
#   auto    — перебор okx → kraken → binance, первый ответивший побеждает
#   okx|kraken|binance — только этот источник
L2_EXTERNAL_SOURCE = _env_str("L2_EXTERNAL_SOURCE", "auto")
L2_EXTERNAL_TIMEOUT_SEC = _env_int("L2_EXTERNAL_TIMEOUT_SEC", 8)
