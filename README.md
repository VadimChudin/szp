...
### Инвалидация и пустые стороны

Касание тенью **не** убирает зону: уровень снимается только пробоем телом H4
(`TEST_INVALIDATES_ZONE=false` по умолчанию; `true` выкашивал нижние зоны
в боковике — регресс «нет зон снизу»). Если в полосе 200-300 пипсов реальных
уровней меньше трёх на стороне, остаток добирается ближайшими реальными
кандидатами за пределами полосы с меткой `EXT` в суффиксе и событием
`zone_added_extended` в журнале. Каждый пересчёт пишет `candidates_summary`
со счётчиком кандидатов сверху/снизу — пустая сторона видна в журнале сразу.
